...
########################################
#betadiv2 <- read.csv("/home/lnkdavi/scratch/rays_beta_all.csv")
betadiv2 <- read.csv("C:/C.drive/Chapter 4/Routputfiles_florent_180222/rays/Cluster_rays_180512/rays_beta_all.csv")
tail(betadiv2)
max(betadiv2$cellid2)

betadiv <- betadiv2[,c("cellid1", "cellid2", "holt.SR")]
head(betadiv)
...
dim(matrix)
matrix[6070:6081,6070:6081]
matrix[1:10,1:10]
matrix2 <- matrix[,-1]
matrixtest <- as.matrix(matrix2)
dim(matrixtest)
matrixtest[1:10, 1:10]
matrixtest[6070:6081,6070:6081]
...
qplot(numclusters, diff, geom = c('point'),
     ...
     size=I(3), data = test) +
  xlab("Number of clusters" ) +ylab( "Rays height difference\nbetween clusters, B") +
  scale_x_continuous(limits = c(0,200))+
  geom_vline(xintercept=c(150), col=I("red"), lwd=I(1)) +
  theme (plot.background = element_rect(fill = "NA", colour = "NA"),
         axis.line.x = element_line(colour = "grey60"),
         panel.grid.major = element_blank(),
         panel.grid.minor = element_blank(),
...
groups <- cutree(clustSR, k=c(1:150))
head(groups)
class(groups)
...
groups2 <- groups2 %>% group_by(X150) %>% dplyr::mutate(countX150=length(X150)) %>% ungroup()
head(groups2)
dim(groups2)
write.csv(groups2,"C:/C.drive/Chapter 4/Routputfiles_florent_180222/rays/raysSRclusters_tomap.csv" )

####################################
#make a list of the cluter numbers that have greater than 10 grid cells
erase<- groups2[ ,c("X150", "Xcellid", "countX150")]
head(erase)
dim(erase)
erase2 <-filter(erase, countX150 <11) #416 cells with less than 11
length(unique(erase2$X150)) #121 clusters with less than 10 grid cells

clusterase <- unique(erase2$X150)
length(clusterase) #121 clusers with less than 10 cells

#search in the groups2 database for those clusters that match these clusterase
#then search back for the max value in the line of code that is not one of the clusterase values
head(groups2)
group4 <- groups2[groups2$X150 %in% clusterase,]
head(group4)
length(unique(group4$X150)) #121
length(unique(group4$Xcellid)) #416

group5 <- melt(group4, id=c("Xcellid", "countX150"))
names(group5)<- c("Xcellid" ,  "countX150", "clustercut" , "clustergroup") 
length(unique(group5$clustergroup)) #different becuase we have many clstergroups now
length(unique(group5$Xcellid)) #416
#get rid of values that aren't clusters anymore
groupsmax <- group5[!group5$clustergroup %in% clusterase,]
head(groupsmax)
length(unique(groupsmax$Xcellid)) #416
length(unique(groupsmax$clustergroup)) #28 which sounds right

#now find the max value of variable and that will mean I get the closest to the 150 break. 
head(groupsmax)
groupsmax$clustercut <- gsub("X", "", paste(groupsmax$clustercut))

groupsmax$clustercut <- as.numeric(groupsmax$clustercut)
head(groupsmax)
groupsmax2 <- groupsmax %>% group_by(Xcellid) %>% dplyr::filter(clustercut == 
              max(clustercut)) %>% ungroup()
length(unique(groupsmax2$Xcellid))
head(groupsmax2)
length(unique(groupsmax2$clustergroup))
badclust <- groupsmax2[,c("clustergroup", "Xcellid")]
names(badclust)<- c("X150","Xcellid")

#now join backinto the database
goodclust <- groups2[!groups2$X150 %in% clusterase,]
goodclust2 <- goodclust[,c("X150","Xcellid")]
head(badclust)
clustfinal <- rbind(badclust, goodclust2)
dim(clustfinal)
length(unique(clustfinal$X150))
count <- clustfinal %>% group_by(X150) %>% dplyr::mutate(countX150=length(X150)) %>% ungroup()

finalclust <- unique(clustfinal$X150)
clusterase
...
dend_h["150"]
clustSR
d <- cut(as.dendrogram(clustSR), h=0.2232097)
d
...
num <- c(3,5,6,7,9,10,11,12,13,15,16,17,20,21,
         22,23,24,25,26,29,30,31,34,35,36,37,
         39,40,43,45,47,49,50,51,52,53,54,57,
         59,60,61)
...
pal(tol21rainbow)
grDevices::col2rgb(tol21rainbow)

#export as letter
duppernode2 %>%set("branches_k_color", value=tol21rainbow, k = 20) %>%
  set("branches_lwd", 8) %>% plot(main = "Ray PD")
