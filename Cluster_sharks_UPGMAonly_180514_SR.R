...
qplot(numclusters, diff, geom = c('point'),
  ...
  geom_vline(xintercept=c(56), col=I("red"), lwd=I(1)) +
  theme (plot.background = element_rect(fill = "NA", colour = "NA"),
         axis.line.x = element_line(colour = "grey60"),
         panel.grid.major = element_blank(),
         panel.grid.minor = element_blank(),
...
groups <- cutree(clustSR, k=c(50,51,52,53,54,55,56))
head(groups)
class(groups)
...
groups2 <- groups2 %>% group_by(X56) %>% dplyr::mutate(countX37=length(X56)) %>% ungroup()
head(groups2[,15:24])
erase<- groups2 %>% group_by(X56) %>% dplyr::summarise(countX56=length(X56)) #put in results
head(erase)
write.csv(groups2,"C:/C.drive/Chapter 4/Routputfiles_florent_180222/sharks/sharkSRclusters_tomap.csv" )
...
tail(dend_h)
k=56
dend_h["56"] #what is the height at the 37th cluster
...
d <- cut(as.dendrogram(clustSR), h=0.2438443 )
plot(d$upper) #this is the plot of the 56 clusters,
...
num <- c(3:4,6:11,13:26,28:33,35:40,42:43,46:47,51:52,54:57)
...
write.csv(duppernodes,"C:/C.drive/Chapter 4/Routputfiles_florent_180222/sharks/sharks.sr.nodes.csv" )
...
abline(h= 0.2438443, col="blue")

firstcol <- "#045A8DFF"
# brightgreen <- "#A3FF73FF"
# col<- viridisLite::viridis(13,  option = "plasma")
# col2 <-viridisLite::viridis(20, option = "viridis")
# col3<- c(brightgreen, col2[11:12], firstcol, col[4:14] )
# col3
# grDevices::col2rgb(col3)
cols <- RColorBrewer::brewer.pal(11,name="Spectral" )
head(cols)
grDevices::col2rgb(cols)
firstcol <- "#045A8DFF"
seccolours <- rev(cols[7:11])
x <- c("253 174 97","244 109 67","218 90 106","198 65 125", "187 52 136")
third <- sapply(strsplit(x, " "), function(x)
  rgb(x[1], x[2], x[3], maxColorValue=255))
fourthcolours <- cols[c(2,1)]
col3 <- c(firstcol, seccolours, third , fourthcolours)
col3
RColorBrewer::display.brewer.pal(11,name="Spectral") 
#export as letter
duppernode2 %>%set("branches_k_color", value=col3, k = 13) %>%
  set("branches_lwd", 8) %>% plot(main = "Shark tax")
